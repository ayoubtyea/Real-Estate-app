...
class PropertyForm(FlaskForm):
    title = StringField(
        ...
        render_kw={"placeholder": "Enter the title of the property"},
    )
    description = TextAreaField(
        "Description",
        validators=[DataRequired(), Length(min=10, max=500)],
        render_kw={"placeholder": "Enter a detailed description of the property"},
    )
    current_price = DecimalField(
        "Current Price",
        validators=[DataRequired(), NumberRange(min=0)],
        render_kw={"placeholder": "Enter the current price of the property"},
    )
    location = StringField(
        "Location",
        validators=[DataRequired(), Length(min=5, max=100)],
        render_kw={"placeholder": "Enter the location of the property"},
    )
    category = StringField(
        "Category",
        validators=[DataRequired(), Length(min=3, max=50)],
        render_kw={"placeholder": "Enter the category of the property"},
    )
    image_url = FileField(
